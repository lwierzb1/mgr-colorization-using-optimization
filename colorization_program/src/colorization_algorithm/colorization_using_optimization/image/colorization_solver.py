...
from colorization_program.src.colorization_algorithm.colorization_using_optimization.image import optimization_solver, \
    weights_cpu_solver
from colorization_program.src.toolkit import mathematical as math_toolkit, image_processing as image_toolkit
...
def compute_weights_of_y_neighbor_values(neighbor_values, y_value):
    ...
    variance = math_toolkit.compute_variance(neighbor_values)
    ...
    wrs = - (wrs / math_toolkit.ensure_is_not_zero(summed_values))
    return wrs
...
class ColorizationSolver:
    ...
    def __init__(self, grayscale_bgr_matrix, marked_bgr_matrix):
        ...
        self._weights_solver = weights_cpu_solver.WeightsCpuSolver()
    ...
    def solve(self):
        ...
        return image_toolkit.yuv_channels_to_bgr_matrix(y_channel, new_u, new_v)

    def _get_yuv_channels_from_matrices(self):
        y_channel, _, _ = image_toolkit.bgr_to_yuv_channels(self._grayscale_bgr_matrix)
        _, u_channel, v_channel = image_toolkit.bgr_to_yuv_channels(self._marked_bgr_matrix)
