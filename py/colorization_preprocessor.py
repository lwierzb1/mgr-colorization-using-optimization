...
class ColorizationPreprocessor:
    ...
    def prepare_input_for_colorization(self, colorized_cells, bw_matrix, color_matrix):
        bw_gridded = self.__create_grid(bw_matrix)
        color_gridded = self.__create_grid(color_matrix)
        min_x, min_y, max_x, max_y = self.__get_mbb(colorized_cells)
