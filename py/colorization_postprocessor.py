...
class ColorizationPostprocessor:
    def post_process(self, mbb, original_bw, input_bw, output_colored):
        propagate_up, propagate_right, propagate_down, propagate_left = self.__check_if_matrix_is_colored_on_edges(
    ...
    @staticmethod
    def __check_if_matrix_is_colored_on_edges(input_bw, colored_matrix):
        diff = cv2.cvtColor(colored_matrix.copy(), cv2.COLOR_BGR2GRAY) - cv2.cvtColor(colored_matrix, cv2.COLOR_BGR2GRAY)
